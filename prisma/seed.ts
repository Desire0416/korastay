...
async function main() {
  ...
  const activitySpecs: { name: string; slug: string; category: string; city: string; citySlug: string; durationHours: number; price: number; min: number; max: number; difficulty: string; included: string; meetingPoint: string; description: string }[] = [
  ...
  for (const a of activitySpecs) {
    await prisma.activity.create({
      data: {
        ...
        destinationId: dest[a.citySlug] ?? null,
        ...
        ratingAverage: 4.6 + Math.random() * 0.4, ratingCount: 5 + Math.floor(Math.random() * 30),

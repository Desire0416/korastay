  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-6">
        ...
        <p className="mt-1 text-muted">Voici un apercu de vos sejours et favoris.</p>
      </div>

      <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
        <KpiCard label="Sejours a venir" value={stats.upcoming} icon={CalendarCheck} tone="brand" href="/account/bookings?when=upcoming" />
        <KpiCard label="Sejours effectues" value={stats.completed} icon={CheckCircle2} tone="success" href="/account/bookings?when=past" />
      ...
      <div className="mt-8 flex items-center justify-between">
        <h2 className="text-xl font-bold text-foreground">Prochains sejours</h2>
      ...
      <div className="mt-4 space-y-3">
        {upcoming.length > 0 ? (
          ...
          <EmptyState
            icon={Compass}
            title="Aucun sejour a venir"
            description="Explorez nos residences verifiees et nos packs pour planifier votre prochain voyage."
            action={
              <div className="flex flex-wrap justify-center gap-3">
                <Button asChild><Link href="/residences"><Home className="h-4 w-4" /> Voir les residences</Link></Button>
      ...
      <div className="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-3">
        {[
          ...
          { href: "/account/reviews", icon: Star, title: "Mes avis", desc: "Partagez vos experiences" },
          { href: "/account/profile", icon: CheckCircle2, title: "Mon profil", desc: "Gerez vos informations" },
        ].map((q) => (

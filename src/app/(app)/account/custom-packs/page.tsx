  ...
  return (
    <div className="mx-auto max-w-3xl">
      <PageHeader
        ...
        description="Vos paniers d'expériences composes sur mesure."
        actions={<Button asChild><Link href="/packs/custom"><Wand2 className="h-4 w-4" /> Composer un pack</Link></Button>}
      />
      {requests.length === 0 ? (
        <EmptyState
          icon={Wand2}
          title="Aucun pack personnalisé"
          description="Composez un séjour sur mesure avec les activités et partenaires d'une ville."
            ...
            <div key={r.id} className="rounded-3xl border border-border bg-surface p-5 shadow-soft">
              ...
              {r.status === "QUOTED" && r.quoteAmount != null && (
                <div className="mt-4 rounded-2xl border border-brand-300 bg-brand-50/50 p-4">
                  <p className="flex items-center gap-2 text-sm font-bold text-foreground"><FileText className="h-4 w-4 text-brand-600" /> Devis reçu</p>
              ...
              {r.reservationId && (
                <div className="mt-4 flex items-center justify-between gap-3 rounded-2xl border border-success/30 bg-emerald-50 p-3">
                  <span className="flex items-center gap-2 text-sm font-semibold text-success"><CheckCircle2 className="h-4 w-4" /> Pack confirmé</span>
                  <Button asChild variant="outline" size="sm"><Link href={`/account/bookings/${r.reservationId}`}>Voir ma réservation</Link></Button>
                </div>
              )}
            </div>
          ))}

...
export async function submitCustomPack(input: CustomPackInput): Promise<CustomPackResult> {
  ...
  if (!input.serviceIds || input.serviceIds.length === 0) return { ok: false, error: "Selectionnez au moins une activite ou un service." };
  ...
  if (admins.length) {
    await prisma.notification.createMany({
      data: admins.map((a) => ({
        userId: a.id, title: "Nouveau pack personnalise",
  ...
  await prisma.notification.createMany({
    data: partnerUserIds.map((uid) => ({
      ...
      body: `Un voyageur vous a inclus dans un pack personnalise a ${input.cityName}.`,
...
export async function sendCustomPackQuote(id: string, _prev: CustomPackResult, formData: FormData): Promise<CustomPackResult> {
  ...
  if (req.userId) {
    await prisma.notification.create({ data: { userId: req.userId, title: "Devis pour votre pack personnalise", body: `Votre pack ${req.reference} : devis de ${amount} F CFA disponible.`, type: "CUSTOM_PACK", url: "/account/custom-packs" } });
  }
...
export async function acceptCustomPackQuote(id: string): Promise<CustomPackResult> {
  ...
  if (req.userId !== user.id && !staff) return { ok: false, error: "Acces refuse." };
  ...
  revalidatePath("/account/custom-packs");
  revalidatePath(`/admin/custom-packs/${id}`);
  return { ok: true, message: "Devis accepte ! Notre equipe finalise votre pack." };
}
...
export async function convertCustomPackToReservation(id: string): Promise<CustomPackResult> {
  ...
  if (!req) return { ok: false, error: "Introuvable." };
  if (req.reservationId) return { ok: false, error: "Ce pack est deja converti en reservation." };
  if (!req.userId) return { ok: false, error: "Le voyageur doit avoir un compte KoraStay pour generer une reservation." };
  ...
  const reservation = await prisma.reservation.create({
    data: {
      ...
      notes: `Pack personnalise ${req.reference} a ${req.cityName}`,
  ...
  for (const item of req.items) {
    ...
    await prisma.partnerMission.create({
      data: {
        ...
        title: item.label, description: `Mission issue du pack personnalise ${req.reference}.`,
    ...
    if (partner) {
      await prisma.notification.create({ data: { userId: partner.userId, title: "Nouvelle mission confirmee", body: `Mission "${item.label}" a ${req.cityName} (pack ${req.reference}).`, type: "PARTNER_MISSION", url: "/partner/missions" } });
    }
  }

  await prisma.customPackRequest.update({ where: { id }, data: { status: "CONFIRMED", reservationId: reservation.id } });
  await prisma.notification.create({ data: { userId: traveler.id, title: "Votre pack personnalise est confirme", body: `Votre pack ${req.reference} est confirme. Reference reservation : ${reservation.reference}.`, type: "RESERVATION_CONFIRMED", url: `/account/bookings/${reservation.id}` } });
  ...
  revalidatePath("/admin/custom-packs");
  revalidatePath(`/admin/custom-packs/${id}`);
  return { ok: true, id: reservation.id, message: "Pack converti en reservation et missions partenaires creees." };
}
...
export async function setCustomPackStatus(id: string, status: string): Promise<CustomPackResult> {
  ...
  if (req.userId) {
    await prisma.notification.create({
      data: { userId: req.userId, title: "Mise a jour de votre pack personnalise", body: `Votre pack ${req.reference} : ${status}.`, type: "CUSTOM_PACK", url: "/account/custom-packs" },
    });
  }
